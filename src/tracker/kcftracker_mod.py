import numpy as np
import cv2
import src.tracker.fhog as fhog
...
class KCFTracker:
    ...
    def get_pred(self, image):
        ...
        # print(self._roi[0], self._roi[1])
        # print('pred', self._roi)
        return self._roi

    def update(self, image, detec_loc=None, use_detec=False):
        if use_detec:
            # Use detection result to update
            self._roi[0] = detec_loc[0]
            self._roi[1] = detec_loc[1]
            self._roi[2] = detec_loc[2] - detec_loc[0]
            self._roi[3] = detec_loc[3] - detec_loc[1]
        # else:
        #     self._roi[0] = int(self._roi[0] + 0.5)
        #     self._roi[1] = int(self._roi[1] + 0.5)
        #     self._roi[2] = int(self._roi[2] + 0.5)
        #     self._roi[3] = int(self._roi[3] + 0.5)
